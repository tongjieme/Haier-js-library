...
if($.fn.tooltipster !== undefined) {
	...
	formValid.showAjaxErrors = function($form, data, isScroll, func){
		var array;
		if (data.Data && ((typeof data.Data.errorInfo === 'string' && data.Data.errorInfo.length > 0) || typeof data.Data.errorInfo === 'object' && data.Data.errorInfo.constructor === Array)) {
	        array = data.Data.errorInfo;
	    } else {
	    	return true;
	    }
		...
		var errorInfo = typeof array === 'object' && array.constructor === Array ? array : eval(array);

		if(errorInfo.length === 0) {
			return true;
		}
		var $to;
	    $.each(errorInfo, function(k,v){
	        var $el = $form.find('[name]').filter(function() {
					    return $(this).attr('name').toLowerCase() === v.LabelName.toLowerCase();
					});
	        ...
	        formValid.tooltips.error($el, msg);
