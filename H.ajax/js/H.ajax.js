...
$.ajaxSetup({
    beforeSend : function () {
        ...
        if (_this.disabledBtn) {
            ...
            _this.disabledBtn.addClass('ui-btn-disabled');
        }
    },
    complete : function () {
        H._ajaxCompleteDo.call(this);
    }
});

H._ajaxCompleteDo = function () {
    var _this = this;
    if (_this.disabledBtn) {
        _this.disabledBtn.each(function () {
            this.disabled = false;
        });
        _this.disabledBtn.removeClass('ui-btn-disabled');
    }
};
...
H.ajax = function (opts) {
    ...
    for(var i = 0; i < mustList_length; i++){
        if (typeof opts[mustList[i]] == 'undefined') {
            H.log('【H.ajax.err】缺少必要参数：' + mustList[i]);
            return false;
        }
    }
